...
| 指标 | 怎么测 | 说明 |
|---|---|---|
| opportunity 检出准确率 | 合成漏斗，已知最大流失点，看是否命中 | 已有 |
| 预算约束捕获率 | 故意生成超预算方案，看 Critic 是否拦下 | 已有 |
| 预算约束误报率 | 合规方案，看是否被误拦 | 已有 |
| 数字幻觉捕获率 | 篡改 opportunity 引用的漏斗数字，看是否被识破 | 已有 |
| 模拟方向准确率 | 只看方向对不对，不看数值 | 已有 |
| **检索 recall@5 / nDCG@5** | 50 题人工标注集 | 🔨 Phase 3 |
| **faithfulness** | 生成的假设是否忠于检索到的案例 | 🔨 Phase 3 |
| **工具调用成功率 / P95 延迟 / 单次成本** | Langfuse trace 统计 | 🔨 Phase 3 |

### 消融实验（Phase 3 产出，README 里放这张表）

| 配置 | recall@5 | faithfulness | 备注 |
|---|---|---|---|
| 无 RAG（纯 LLM 生成） | — | ? | 基线 |
| 纯向量检索 | ? | ? | |
| 混合检索（BM25 + 向量） | ? | ? | |
...
### ⚠️ 关于 report.json 当前全是 1.0

**这是目前最大的可信度风险，面试官一定会问。两条路：**

**路线 A（推荐，Phase 3 做掉）**：把评测做难。
- 加 20 条对抗性用例：预算刚好超 0.5%、漏斗数字偏差 1% 以内、引用不存在的环节、逻辑自洽但违背常识的方案
- 让 `experiment_agent` 用真实 LLM 生成方案，而不是现在 orchestrator 里那个确定性的 `min(budget, limit)` clamp
- 目标是把分数打到 0.75–0.9 这个**可信区间**

**路线 B（万一来不及）**：诚实解释。
> 「这五个指标目前是满分，但我要说明这个满分的含金量有限：
> Critic 现在是规则校验——预算比大小、漏斗数字重算——对这类确定性错误确实是 100% 捕获。
> 而测试用例来自我自己写的合成数据生成器，本质上是自己考自己。
> 它能证明的是'规则层没有实现 bug'，不能证明'Agent 在真实场景下可靠'。
> 我下一步要加的是对抗性用例和真实 LLM 生成的方案，我预期分数会掉到 0.8 左右，那个数字才有参考价值。」

**说出这段话的加分，远大于一个 1.0 的分数。**
...
| 短板 | 现状 | 我的计划 |
|---|---|---|
| **评测集偏易** | report.json 五项全 1.0，规则校验 + 自造数据自己考自己 | 加 20 条对抗性用例 + 真实 LLM 生成方案，预期降到 0.8 |
